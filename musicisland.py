...
class MiniMusicPlayer(QWidget):
    ...
    def _start_global_hotkey_listener(self):
        ...
        try:
            ...
            # Track VK codes successfully claimed by RegisterHotKey so the
            # LL-hook fallback does not fire a duplicate signal for them.
            _registered_vks = set()
                ...
                try:
                    if nCode == 0 and wParam in (WM_KEYDOWN, WM_SYSKEYDOWN):
                        ...
                        if vk in _watched_vks and vk not in _key_pressed:
                            _key_pressed.add(vk)
                            # Skip keys already handled by RegisterHotKey to
                            # avoid double-firing (except drag which is always
                            # handled here).
                            if vk not in _registered_vks:
                                _emit_for_vk(vk)
    ...
    def _start_crossfade(self, file_path: str, target_index: int):
        try:
            ...
            if getattr(self, '_crossfade_timer', None) is not None:
                try:
                    # Preserve current_index: _finish_crossfade overwrites it
                    # with the old target, which would confuse next/prev_track
                    # when the user presses rapidly.
