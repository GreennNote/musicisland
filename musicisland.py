...
def load_cover_asset_path():
    """Resolve no-song cover image path for dev and onefile builds."""
    candidates = [
...
from PyQt6.QtGui import QPainter, QPolygon, QColor, QPixmap, QLinearGradient, QRadialGradient, QPainterPath, QMouseEvent, QCursor, QGuiApplication, QImage, QFontDatabase, QIcon, QRegion
...
class EdgeEnvironmentSampler:
    ...
    def __init__(self, fps: int = 30, thickness: int = 24, outward_offset: int = 3):
        ...
        self._front = {'top': None, 'bottom': None, 'left': None, 'right': None, 'stamp': 0.0}
        self._backend = None
        self._dxcam = None
        self._mss = None
    ...
    def sample_edge_color(self, side: str, u: float = 0.5, v: float = 0.5, blur_radius: int = 1, distortion: float = 0.01, t: float = None) -> QColor:
        ...
        # tiny animated offset to mimic liquid distortion
        off = (math.sin((u * 17.0) + (t * 1.4)) + math.cos((v * 13.0) + (t * 1.1))) * distortion

        iw = max(1, img.width() - 1)
        ih = max(1, img.height() - 1)
        if side in ('top', 'bottom'):
            sx = max(0, min(iw, int((u + off) * iw)))
            sy = max(0, min(ih, int(v * ih)))
        else:
            sx = max(0, min(iw, int(v * iw)))
            sy = max(0, min(ih, int((u + off) * ih)))

        # Fast path: no blur — sample single pixel
        br = max(0, int(blur_radius))
        if br == 0:
            c = img.pixelColor(sx, sy)
            return QColor(c.red(), c.green(), c.blue())

        # Use pixel() (returns ARGB int) instead of pixelColor() for speed
        rr = gg = bb = cnt = 0
        w_max = img.width() - 1
        h_max = img.height() - 1
        for dy in range(-br, br + 1):
            yy = max(0, min(h_max, sy + dy))
            for dx in range(-br, br + 1):
                xx = max(0, min(w_max, sx + dx))
        ...
        if cnt <= 0:
            return QColor(255, 255, 255)
        return QColor(rr // cnt, gg // cnt, bb // cnt)
    ...
    def _capture_loop(self):
        ...
        while self._running:
            ...
            for side, rect in rects.items():
                ...
                tw = 64 if side in ('top', 'bottom') else 8
                th = 8 if side in ('top', 'bottom') else 64
                try:
    ...
    class _PycawVolume:
        def __init__(self):
            ...
            try:
                ...
                self._endpoint = comtypes.cast(interface, comtypes.POINTER(IAudioEndpointVolume))
            except Exception:
                self._endpoint = None
...
class TutorialPopup(QWidget):
    ...
    def _build_ui(self):
        ...
        # --- F11 row with drag icon ---
        f11_row = QHBoxLayout()
        f11_row.setSpacing(8)
        f11_key = self._key_graphic("F11")
        f11_row.addWidget(f11_key)
        ...
        try:
            ...
            pass
        f11_row.addWidget(drag_lbl)
        f11_lbl = QLabel("Hold to drag. In small mode snaps\nto nearest window when released.")
        f11_lbl.setWordWrap(True)
        f11_lbl.setStyleSheet("font-size:12px;color:rgba(255,255,255,210);background:transparent;")
        f11_row.addWidget(f11_lbl, 1)
        inner.addLayout(f11_row)
    ...
    def paintEvent(self, event):
        ...
        if self.cover_pixmap:
            ...
            try:
                ...
                grad.setColorAt(0.0, c1)
                grad.setColorAt(0.7, QColor(glow_color.red(), glow_color.green(), glow_color.blue(), 40))
                grad.setColorAt(1.0, c2)
                from PyQt6.QtGui import QBrush
                painter.setPen(Qt.PenStyle.NoPen)
                painter.setBrush(QBrush(grad))
            ...
            try:
                ...
                try:
                    ...
                    g.setColorAt(0.0, base_col)
                    try:
                        end_col = QColor(base_col)
                        end_col = end_col.darker(180)
                        g.setColorAt(1.0, end_col)
                    except Exception:
...
class MarqueeLabel(QLabel):
    ...
    def _update_timer(self):
        try:
            ...
            try:
                if hasattr(self, '_text_width') and self._text_width > available:
...
class PlaylistItemWidget(QWidget):
    ...
    def _finish_inline_rename(self):
        if self._edit is None:
            return
        try:
            new_text = self._edit.text().strip() or self._text
            self._text = new_text
            self.label.setText(new_text)
            # restore label in layout
            layout = self.layout()
            try:
                layout.removeWidget(self._edit)
            except Exception:
                pass
            try:
...
class MiniMusicPlayer(QWidget):
    ...
    def __init__(self):
        ...
        self._hotkey_toggle_vk = 0x79   # F10 default
        self._hotkey_drag_vk = 0x7A     # F11 default
        self._global_hotkeys = {
            'toggle': {'vk': self._hotkey_toggle_vk, 'mod': 0},
            'next': {'vk': 0x21, 'mod': 0},    # Page Down - next song
            'prev': {'vk': 0x22, 'mod': 0},    # Page Up - previous song
        ...
        # spectral probe state (low <300Hz, high >=300Hz)
        ...
        try:
            ...
            pass
        try:
            self._edge_sampler = EdgeEnvironmentSampler(fps=15, thickness=36, outward_offset=5)
    ...
    def _init_ui_components(self):
        # Import button
        self.import_btn = GlassButton("Import Music", self, reflection_radius=8.0, reflection_intensity=0.18)
    ...
    def _setup_complete_ui(self):
        ...
        try:
            ...
            _vol_shadow = QGraphicsDropShadowEffect(self.volume)
            _vol_shadow.setBlurRadius(18)
            _vol_shadow.setOffset(0, 0)
            _vol_shadow.setColor(QColor(0, 0, 0, 140))
            self.volume.setGraphicsEffect(_vol_shadow)
        except Exception:
            pass
    ...
    def _start_global_hotkey_listener(self):
        # Register multiple system-wide hotkeys on Windows that emit signals when pressed.
        try:
            import ctypes
            from ctypes import wintypes
            import threading
                        ...
                        try:
                            if user32.PeekMessageA(ctypes.byref(msg), None, 0, 0, 1):
                                if msg.message == WM_HOTKEY:
                                    ...
                                    action = hotkey_ids.get(hotkey_id)
                                    if action:
                                        if action == 'toggle':
    ...
    def stop_playlist_drag(self):
        ...
        try:
            ...
            # compute final drop position based on current mouse position
            pos = QCursor.pos()
            local = self.list_widget.mapFromGlobal(pos)
            
            # compute target row using indexAt for robustness
            count = self.list_widget.count()
            if count == 0:
                target = 0
            else:
                idx = self.list_widget.indexAt(local)
                if idx.isValid():
                    row = idx.row()
                    # decide whether to insert above or below the item based on y position
                    rect = self.list_widget.visualItemRect(self.list_widget.item(row))
                    if local.y() > rect.center().y():
                        target = row + 1
                    else:
                        target = row
                else:
                    # if outside any item, drop at end or start depending on y
                    if local.y() < 0:
                        target = 0
                    else:
                        target = count
                ...
                if event.type() == QEvent.Type.MouseMove:
                    ...
                    if count > 0:
                        ...
                        for i in range(count):
                            r = self.list_widget.visualItemRect(self.list_widget.item(i))
                            centers.append((i, r.center().y()))
                        y = local.y()
                        best = min(centers, key=lambda t: abs(t[1] - y))[0]
                        try:
    ...
    def _on_volume_changed(self, v: int):
        try:
            ...
            # Apply logarithmic scaling for more linear volume perception
            # Convert linear 0-100 to logarithmic 0-1
            if v == 0:
                volume = 0.0
            else:
                # Logarithmic scale: map 0-100 to 0-1 using log curve
                # This makes low volume changes more gradual and high volume changes more noticeable
                volume = (v / 100.0) ** 1.5  # Power curve for logarithmic feel
            
            # Ensure volume is in valid range
            volume = max(0.0, min(1.0, volume))
            
            # set application audio volume - self.audio is always the active output
            try:
        ...
        try:
            ...
            if os.path.exists(settings_file):
                with open(settings_file, 'r') as f:
                    ...
                    self._hotkey_toggle_vk = settings.get('hotkey_toggle_vk', 0x79)
                    self._hotkey_drag_vk = settings.get('hotkey_drag_vk', 0x7A)
                    try:
    ...
    def _save_settings(self):
        ...
        try:
            ...
            settings = {
                ...
                'hotkey_toggle_vk': getattr(self, '_hotkey_toggle_vk', 0x79),
                'hotkey_drag_vk': getattr(self, '_hotkey_drag_vk', 0x7A),
            }
    ...
    def open_settings(self):
        ...
        try:
            ...
            dlg.setModal(False)
            dlg.setFixedSize(350, 610)  # Height for all settings groups
            ...
            reset_color_btn.setStyleSheet("QPushButton { background-color: rgba(0,0,0,180); color: white; border: none; border-radius: 0px; padding: 6px; } QPushButton:hover { background-color: rgba(40,40,40,200); }")
            ...
            upload_cover_btn.setStyleSheet("QPushButton { background-color: rgba(0,0,0,180); color: white; border: none; border-radius: 0px; padding: 6px; } QPushButton:hover { background-color: rgba(40,40,40,200); }")
            ...
            remove_cover_btn.setStyleSheet("QPushButton { background-color: rgba(0,0,0,180); color: white; border: none; border-radius: 0px; padding: 6px; } QPushButton:hover { background-color: rgba(40,40,40,200); }")
                ...
                try:
                    ...
                    def do_reset():
                        try:
                            ...
                            self._hotkey_toggle_vk = 0x79
                            self._hotkey_drag_vk = 0x7A
                            self._global_hotkeys['toggle']['vk'] = 0x79
                            try:
                                self._stop_global_hotkey_listener()
                                self._start_global_hotkey_listener()
                            except Exception:
                            ...
                            # Reset UI elements
                            try:
                                self._set_song_text_with_font("No song playing")
                                self.cover.set_cover(None)
            ...
            if custom_path and isinstance(custom_path, str) and os.path.exists(custom_path):
                ...
                if pm and not pm.isNull():
                    ...
                    if w > h:
                        side = h
                        x = max(0, (w - side) // 2)
                        cover_pix = pm.copy(x, 0, side, side)
                    else:
                        cover_pix = pm
        ...
        try:
            ...
            for tag in tags.values():
                if isinstance(tag, APIC):
                    ...
                    # center-crop landscape to square
                    w = pix.width()
                    h = pix.height()
                    if w > h:
                        side = h
                        x = max(0, (w - side) // 2)
                        cover_pix = pix.copy(x, 0, side, side)
                    else:
                        cover_pix = pix
                    ...
                    # Apply the existing theme color (set at import time or by user)
                    if file_path in self._theme_colors:
                        self._set_theme_color_for_song(file_path, self._theme_colors[file_path])
                    else:
                        # Fallback: sample colors from cover art (should not normally happen)
            ...
            # if no APIC frame found, try common external image files next to the mp3
            if not found_apic:
                try:
                    d = os.path.dirname(file_path) or '.'
                    base = os.path.splitext(os.path.basename(file_path))[0]
                    # Only accept an *exact* per-track image (songname.jpg/png/jpeg).
                    # Ignore generic filenames like cover.jpg/folder.png which caused
                    # unrelated/stretched artwork to be used previously.
    ...
    def play_index(self, index):
        ...
        # clear existing cover immediately to avoid showing a stale image
        try:
            self.cover.set_cover(None)
        except Exception:
            pass
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                # attempt to load cover immediately (may update during crossfade)
    ...
    def _start_crossfade(self, file_path: str, target_index: int):
        try:
            ...
            target_vol = max(0.0, min(1.0, (self.volume.value() if hasattr(self, 'volume') else 70) / 100.0))
    ...
    def _update_crossfade(self):
        try:
            ...
            if to_audio is not None:
                try:
                    to_audio.setVolume(max(0.0, min(1.0, (self.volume.value() if hasattr(self, 'volume') else 70) / 100.0) * t))
                except Exception:
                    pass
    ...
    def _fade_in_song_ui(self):
        try:
            ...
            # Create smoother, longer fade animations
            ...
            # Start animations with slight delay for visual appeal
            if a1 and a2:
                self._fade_anim = (a1, a2)
                a1.start()
                # Start cover animation slightly later for a staggered effect
                from PyQt6.QtCore import QTimer
                QTimer.singleShot(50, a2.start)
            elif a1:
                a1.start()
                self._fade_anim = (a1,)
            elif a2:
                ...
                # Fallback to original method if animation creation fails
    ...
    def _start_close_animation(self):
        ...
        duration_ms = 206  # additional 1.7x faster than previous timing
        start_rect = self.geometry()
        ...
        try:
            ...
            try:
                # InOutSine gives a softer start/end than cubic for close motion.
                self._close_anim.setEasingCurve(QEasingCurve(QEasingCurve.Type.InOutSine))
            except Exception:
                pass
            self._close_anim.setStartValue(start_rect)
            try:
                # Hold shape briefly, then glide toward the final compact state.
                self._close_anim.setKeyValueAt(0.18, start_rect)
            except Exception:
                pass
        ...
        # Fade window opacity together with geometry for a smoother perceived close.
        try:
        ...
        try:
            ...
            try:
                # Fade starts gently and finishes smoothly to avoid a sudden disappear.
            ...
            try:
                self._close_opacity_anim.setKeyValueAt(0.12, max(0.0, float(self.windowOpacity())))
                self._close_opacity_anim.setKeyValueAt(0.55, 0.42)
            except Exception:
                pass
        ...
        # Resolve currently active audio output (setup code may use either name).
        active_audio = None
        try:
            active_audio = self.player.audioOutput() if getattr(self, 'player', None) is not None else None
        except Exception:
            active_audio = None
        if active_audio is None:
            try:
                active_audio = getattr(self, 'audio_output', None)
            except Exception:
                active_audio = None
        if active_audio is None:
            try:
                active_audio = getattr(self, 'audio', None)
            except Exception:
                active_audio = None

        # If music is playing, fade volume and pitch down while closing.
        playing = False
        try:
            playing = self.player.playbackState() == QMediaPlayer.PlaybackState.PlayingState
        except Exception:
        ...
        if not playing:
            try:
                # Fallback: treat as playing if a track is loaded and has advanced.
                playing = self.player.position() > 0 and self.player.source().isLocalFile()
            except Exception:
                playing = False

        if playing and active_audio is not None:
            try:
            ...
            try:
                start_rate = float(self.player.playbackRate())
            except Exception:
                start_rate = 1.0
            if start_rate <= 0.0:
                start_rate = 1.0
            ...
            try:
                self._close_restore_volume = float(active_audio.volume())
            except Exception:
                self._close_restore_volume = 1.0

            try:
                self._close_audio_anim = QVariantAnimation(self)
                self._close_audio_anim.setDuration(max(253, duration_ms + 65))
                try:
                    self._close_audio_anim.setEasingCurve(QEasingCurve(QEasingCurve.Type.InOutSine))
                except Exception:
                    pass
                self._close_audio_anim.setStartValue(0.0)
                self._close_audio_anim.setEndValue(1.0)

                def _on_audio_step(v):
                    try:
                        t = max(0.0, min(1.0, float(v)))
                        base_vol = self._close_restore_volume if self._close_restore_volume is not None else 1.0
                        # smooth weighted fade-out
                        active_audio.setVolume(max(0.0, base_vol * ((1.0 - t) ** 1.2)))
                    except Exception:
                        pass
                    try:
                        t = max(0.0, min(1.0, float(v)))
                        # Keep pitch-down in a backend-safe range to avoid cutouts.
                        # Windows multimedia backends often mute/drop at very low rates.
                        target_rate = 0.55 if sys.platform == 'win32' else 0.35
                        shaped = t ** 1.6
                        rate = start_rate + (target_rate - start_rate) * shaped
                        self.player.setPlaybackRate(max(target_rate, rate))
                    except Exception:
                        pass

                self._close_audio_anim.valueChanged.connect(_on_audio_step)
            except Exception:
                self._close_audio_anim = None

        close_state = {
            'geom_done': False,
            'opacity_done': (self._close_opacity_anim is None),
            'audio_done': (not playing) or (self._close_audio_anim is None)
        }

        def _maybe_finish_close():
            if not (close_state.get('geom_done') and close_state.get('opacity_done') and close_state.get('audio_done')):
                return
            try:
                if playing:
                    try:
        ...
        def _on_geom_finished():
            close_state['geom_done'] = True
            _maybe_finish_close()

        def _on_opacity_finished():
            close_state['opacity_done'] = True
            try:
                self.hide()
            except Exception:
                pass
        ...
        def _on_audio_finished():
            close_state['audio_done'] = True
            _maybe_finish_close()

        try:
            if self._close_anim is not None:
                self._close_anim.finished.connect(_on_geom_finished)
        ...
        try:
            if self._close_opacity_anim is not None:
                self._close_opacity_anim.finished.connect(_on_opacity_finished)
        ...
        try:
            if self._close_audio_anim is not None:
                self._close_audio_anim.finished.connect(_on_audio_finished)
    ...
    def paintEvent(self, event):
        ...
        try:
            # don't show the gradient until a song starts playing, but show at 50% when paused
            try:
                playing = False
                ...
                # Track pause state for fading
                current_pause_state = not playing
                last_pause_state = getattr(self, '_last_pause_state', False)
                
                # Initialize fade variables if not present
                if not hasattr(self, '_pause_fade_progress'):
                    self._pause_fade_progress = 0.0  # 0 = fully playing, 1 = fully paused
                    self._last_pause_state = current_pause_state
                
                # Update fade progress based on state change
                if current_pause_state != last_pause_state:
                    # State changed, start fade animation
                    self._last_pause_state = current_pause_state
                
                # Smooth fade animation (60fps, 0.5 second duration)
                fade_speed = 0.02  # 1/50 = 0.02 for ~0.5 second fade
                if current_pause_state:
                    # Fading to paused (50% brightness)
                    if self._pause_fade_progress < 1.0:
                        self._pause_fade_progress = min(1.0, self._pause_fade_progress + fade_speed)
                else:
                    # Fading to playing (100% brightness)
                    if self._pause_fade_progress > 0.0:
                        self._pause_fade_progress = max(0.0, self._pause_fade_progress - fade_speed)
                
                if not playing and self._pause_fade_progress > 0.0:
                    # Show gradient with fade transition
                    base_orig = QColor(self._theme_color)
                    # Interpolate brightness: 1.0 (full) -> 0.5 (paused)
                    bmul = 1.0 - (0.5 * self._pause_fade_progress)
                    # Use static values for energy when paused
                    low_e = 0.1
                    high_e = 0.1
                    react = 1.0
                    
                    # compute endpoint brightness multipliers
                    ...
                    # Create gradient with paused colors
                    ...
                    grad.setColorAt(0.0, QColor(rr1, gg1, bb1, glass_a))
                    grad.setColorAt(1.0, QColor(rr2, gg2, bb2, glass_a))
                    
                    painter.fillRect(self.rect(), grad)
                    painter.end()
                    return
                elif not playing:
                    # Fully paused, show at 50%
                    base_orig = QColor(self._theme_color)
                    bmul = 0.5  # 50% brightness when paused
                    # Use static values for energy when paused
                    low_e = 0.1
                    high_e = 0.1
                    react = 1.0
                    ...
                    # Create gradient with paused colors
                    grad = QLinearGradient(QPointF(0, 0), QPointF(self.width(), self.height()))
                    grad.setColorAt(0.0, QColor(rr1, gg1, bb1))
                    grad.setColorAt(1.0, QColor(rr2, gg2, bb2))
            ...
            base_orig = QColor(self._theme_color)
            bmul = getattr(self, '_auto_brightness', 1.0)
            # spectral energies (smoothed) computed from audio probe
            low_e = max(0.0, min(1.0, getattr(self, '_low_energy', 0.0)))
            high_e = max(0.0, min(1.0, getattr(self, '_high_energy', 0.0)))
            # make top-section gradient react more strongly to audio (5x faster)
            react = max(0.0, float(getattr(self, '_volume_reactivity', 1.0))) * 5.0

            # compute endpoint brightness multipliers; tuned so energies map smoothly
            low_mul = bmul * (1.0 + 1.8 * low_e * react)
            high_mul = bmul * (1.0 + 1.8 * high_e * react)
        ...
        try:
            ...
            try:
                ...
                if arrf.size < 8:
                    ...
                    # Only react to frequencies between 100Hz and 500Hz
                    target_range = (freqs >= 100) & (freqs <= 500)
                    target_energy = mags[target_range].sum() / total
                    raw_low = float(target_energy)
                    raw_high = 0.0  # We don't use high frequencies anymore

                # smooth energies
                alpha = 0.18
                self._low_energy = (1.0 - alpha) * getattr(self, '_low_energy', 0.0) + alpha * raw_low
                self._high_energy = (1.0 - alpha) * getattr(self, '_high_energy', 0.0) + alpha * raw_high
                return
            except Exception:
                pass
